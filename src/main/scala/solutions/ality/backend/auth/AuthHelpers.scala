...
import solutions.ality.backend.auth.AuthHelpers.Role.{Administrator, Customer}
...
object AuthHelpers {
  ...
  val AdminRequired: BasicRBAC[IO, Role, User, AugmentedJWT[HMACSHA256, Int]] = BasicRBAC[IO, Role, User, AugmentedJWT[HMACSHA256, Int]](Administrator)
  val CustomerRequired: BasicRBAC[IO, Role, User, AugmentedJWT[HMACSHA256, Int]] = BasicRBAC[IO, Role, User, AugmentedJWT[HMACSHA256, Int]](Administrator, Customer)

  case class User(id: Int, age: Int, name: String, role: Role = Role.Customer)

  object User {
    implicit def authRole[F[_]](implicit F: MonadError[F, Throwable]): AuthorizationInfo[F, Role, User] = new AuthorizationInfo[F, Role, User] {
